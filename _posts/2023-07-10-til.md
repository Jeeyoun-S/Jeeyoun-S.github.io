...
그렇다면 어떻게 한 가지 역할만 하는 함수를 만들까? 어떤 기준으로? 대부분의 글에서는 '추상화 수준'이 하나여야 한다고 설명하는데, 정확히 추상화 수준이 무엇인지 이해하기 어려웠다. 내가 이해한 바로는 객관적이기보다 주관적이라 느꼈다. 쉽게 설명한 글을 찾았는데, 한 번의 추상화 수준을 한 번의 **들여 쓰기**로 표현해 보라는 것이었다.
