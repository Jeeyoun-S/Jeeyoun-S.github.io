...
쉽게 설명한 글을 찾았는데, 한 번의 추상화 수준을 한 번의 **들여 쓰기**로 표현해 보라는 것이었다. 우선은 들여쓰기를 줄이려는 노력부터 시작해보면 좋을 듯하다.

### 모듈화
나아가 Javascript 파일을 모듈화해서, 함수 용도에 따라 파일을 분리 방법도 알아두면 좋으리라 생각했다. 막상 공부하고 나니 Vue.js에서 수없이 사용했던 방법인 걸 깨달았다. 단일 js 파일에서 작업하는 걸, Vue.js와 너무 다르게 생각하지 말아야겠다는 생각이 들었다.
