...
#### Image File 보여주기
회원 관리 기능을 구현하면서 가장 많은 시간을 썼던 부분이다.
...
겉으로 봤을 때는 완성된 것 같지만, 사실은 많은 오류를 안고 있는 미완성 프로젝트이기도 하다.

1. 화상 상담 시, 화상 상담 URL로 창을 여러 개 띄우면 화면이 띄운 창 개수만큼 늘어난다.
2. 무한 스크롤이나 페이지네이션이 구현돼 있지 않아 10,000건이 넘는 목록을 띄우면 화면이 멈춘다.

#### UI/UX

사이드바도 항상 상단에 고정돼 있어 스크롤을 내렸을 때 사이드바에 접근하기 어려웠다. 예약 일정을 보여줄 때도 달력을 활용했다면 더 좋았으리라는 생각이 들었다.

더불어 웹 페이지에 움직이는 요소가 없어 다소 딱딱하고 정적인 느낌을 주는 것도 아쉬웠다.
...
이 프로젝트의 가장 핵심 기능인 화상 미팅을 팀장님께서 전부 개발하셨다. 먼저 도와드릴 부분이 있는지 묻기도 했지만, 거절당했다. 역할 분담을 할 때 자신이 없어 핵심 기능을 함께 하겠다고 적극적으로 나서지 못한 것이 아쉬움이 남는다. 마지막주까지 화상 미팅이 완성되지 못했었는데, 핵심 기능인만큼 여러 명이 함께 했으면 더 좋은 결과가 있지 않았을까 싶다.
